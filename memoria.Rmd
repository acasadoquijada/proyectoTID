...
reglasGenerales <- apriori(ds[,-c(1, 2, 3, 5, 6, 8, 11, 12, 13, 14, 16, 17, 18, 19, 20, 25, 178, 179, 180)], parameter = list(support = 0.1, confidence = 0.8, minlen = 2, maxlen = 3, maxtime=300))
```
```{r}
...
knnFit <- train(nombre ~ ., data = train, method = "knn", trControl = ctrl, tuneLength = 20 )
```
...
```{r}
library(xgboost)
trainIndex <- createDataPartition(dsFiltrado$nombre, p = .8, list = FALSE, times = 1)
idxNombre = which(colnames(dsFiltrado) == "nombre")

training <- dsFiltrado[trainIndex, -idxNombre]
test <- dsFiltrado[-trainIndex, -idxNombre]

xgb <- xgboost(data = data.matrix(training), label = dsFiltradoClaseNum[trainIndex], eta = 0.1, max_depth = 7, nround=25, subsample = 0.5, colsample_bytree = 0.5, seed = 1, eval_metric = "merror", objective = "multi:softmax", num_class = 5, nthread = 3)
```
