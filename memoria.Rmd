...
```{r}
mapa <- base_world + geom_point(data = ds, aes(x = longitude, y = latitude, color = ds$temperature)) + scale_colour_gradient2(low = "darkblue", mid="yellow", high = "red", midpoint = 25)
mapa
```
...
circunferencia <- ggplot(datos, aes(datos$windSpeed*cos(datos$windBearing), datos$windSpeed*sin(datos$windBearing))) + geom_point(aes(colour = datos$tipo)) + escaleta
circunferencia
```
