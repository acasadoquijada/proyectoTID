...
```{r}
ds$closeToWater <- factor(ds$closeToWater, levels = c("false", "true"))
tabla3 <- table(ds[ds$tipo %in% c("agua","bicho","planta","veneno","fuego"), c("closeToWater", "tipo")])
barplot(tabla3, legend = levels(ds$closeToWater), las = 2, col = c("darkgoldenrod2", "chocolate", "cyan4", "blue4"))
```
Los tipos elegidos han sido los considerados más propensos a aparecer cerca de zonas acuosas, por otro lado también se ha usado el tipo fuego.

Se puede comprobar como el tipo agua tiene un gran número de avistamientos cerca de zonas de agua con respecto a los demás, mientrás que el tipo fuego apenas dispone de avistamientos cerca del agua, como es natural.

Antes hemos estado discutiendo sobre los dias de las apariciones de los diferentes pokemon. Por lo que ahora vamos a ver como se distribuyen dichas apariciones a lo largo de los dias de la semana. Para realizar esto vamos a elegir a los siguientes tipos: písquico, planta, fuego, tierra y roca

```{r}
ds$appearedDayOfWeek <- factor(ds$appearedDayOfWeek, levels = c("Monday", "Friday", "Saturday", "Sunday", "Thursday", "Tuesday", "Wednesday"))
tabla3 <- table(ds[ds$tipo %in% c("psiquico","planta","fuego","tierra","roca"), c("appearedDayOfWeek", "tipo")])
barplot(tabla3,las = 2, col = c("darkgoldenrod2", "chocolate", "cyan4", "blue4","firebrick1","darkolivegreen","deeppink2"))
```
...
Vamos a representar los datos contenidos en nuestro dataset de forma geográfica:

```{r}
world_map <- map_data("world")
spain <- subset(world_map, world_map$region=="Spain")
base_world <- ggplot() + coord_fixed() + xlab("") + ylab("") + geom_polygon(data=world_map, aes(x=long, y=lat, group=group), colour="light green", fill="light green")
...
min(ds[ds$city == "New_York",]$latitude)
max(ds[ds$city == "New_York",]$latitude)
min(ds$latitude)
max(ds$latitude)
print("Veamos que cae en el rango de Madri:")
unique(ds[ds$latitude >= minLat & ds$latitude <= maxLat & ds$longitude >= minLong & ds$longitude <= maxLong,]$city)

madrid <- ds[ds$city == "Madrid",]
unique(ds[ds$cellId_90m %in% unique(madrid$cellId_90m),]$city)
